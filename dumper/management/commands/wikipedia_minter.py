...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        col, _ = Collection.objects.get_or_create(
            name="No german Wikipedia-Site found"
        )

        ents = TempEntityClass.objects.filter(uri__domain="wikidata").\
            exclude(uri__uri__icontains="wikipedia").exclude(collection=col)
        uris_to_process = Uri.objects.filter(entity__in=ents).filter(domain="wikidata")

        print(f"All in all {uris_to_process.count()} WIKIDATA-Entities without Wikipedia")
